...
export const ExchangeReportsClient = () => {
  const [accounts, setAccounts] = useState<AccountWithTokens[]>([]);
  const [users, setUsers] = useState<User[]>([]);
  const [selectedAccountId, setSelectedAccountId] = useState<string>("");
  const [dateRange, setDateRange] = useState<DateRangeValue>(getDefaultDateRange);
  const [report, setReport] = useState<NftExchangeReportBundle | null>(null);
  ...
  const handleFetchReports = async () => {
    ...
    try {
      ...
      const result = await getExchangeReportsAction({
    ...
    <div className="space-y-6">
      ...
      <div>
        <Link href="/admin/tokens">
          <Button variant="outline">返回 Token 管理</Button>
        </Link>
      </div>
